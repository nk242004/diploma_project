...
@admin.action(description='Send messages for user activation')
def send_notifications(modeladmin, request, queryset):
    for rec in queryset:
        if not rec.is_activated:
            send_activation_notification(rec)
    modeladmin.message_user(request, 'Letters have been sent')

class NonactivatedFilter(admin.SimpleListFilter):
    title = 'Did you activate your account'
    parameter_name = 'actstate'

    def lookups(self, request, model_admin):
        return (
                   ('activated', 'activated'),
                   ('threedays', 'account has not been activated for 3 day'),
                   ('week', 'account has not been activated for more than a week'),
               )
